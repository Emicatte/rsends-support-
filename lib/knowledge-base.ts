...
# RSends Support Knowledge Base

## How RSends Works
- RSends is a Web3 B2B payment gateway for European businesses on Base L2 (multi-chain EVM + Tron + Solana planned).
...
## Payments
- Direct payments: Payer selects token/amount/recipient on RSends → Oracle verifies → on-chain via FeeRouter.
...
## Forwarding & Rules
- Auto-forwarding: Payments auto-forwarded to another wallet (cold storage, accounting). Configured in Command Center. Gas paid by RSends.
...
## Fees
- Platform fee: 0.5% per transaction, deducted by FeeRouter before recipient gets paid. Goes to RSends Treasury.
...
- Swap txs (e.g. ETH→USDC) add Uniswap pool fee (~0.3%) on top of RSends 0.5%.
...
- RSends NEVER has access to user private keys, NEVER asks for seed phrases, NEVER initiates txs without user signature.
...
- Webhook signature: X-RSends-Signature header = HMAC-SHA256 of body with webhook secret. Always verify before processing.
`;
